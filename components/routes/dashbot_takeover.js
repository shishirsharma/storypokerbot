...
module.exports = function(webserver, controller) {
  ...
  var handler = {
    ...
    dashbotReceive: function(req, res){
      ...
      if (process.env.DASHBOT_API_KEY) {
        let payload = req.body;
        if(process.env.DASHBOT_API_KEY === payload.apiKey) {
          let teamId = payload.teamId;
          ...
          controller.findTeamById(teamId, (error, team) => {
            ...
            let bot = controller.spawn(team.bot);
  ...
  // Create a /dashbot/pause link
  // This is the link that receives the postback from Slack's oauth system
  // So in Slack's config, under oauth redirect urls,
  // your value should be https://<my custom domain or IP>/oauth
  debug('Configured /dashbot/pause url');
  webserver.post('/dashbot/pause', handler.dashbotPause);

  // Create a /dashbot/receive link
  // This is the link that receives the postback from Slack's oauth system
  // So in Slack's config, under oauth redirect urls,
  // your value should be https://<my custom domain or IP>/oauth
  debug('Configured /dashbot/receive url');
  webserver.post('/dashbot/receive', handler.dashbotReceive);
