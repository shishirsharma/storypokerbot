...
module.exports = function(controller) {
  ...
  controller.on('interactive_message_callback', function(bot, trigger) {
    ...
    //     return false; // do not bubble event
    // }
    if (trigger.callback_id.match(/^select_poker_action_modfib$/)) {
      ...
      if (trigger.actions[0].name.match(/^Reveal$/)) {
        ...
        let graph = {
          ...
          '13': 0, '20': 0, '40': 0, '100': 0, 'Break Pls': 0 
        };
        ...
        reply.attachments[0].image_url = `https://chart.googleapis.com/chart?cht=bvs&chs=480x270&chd=t:${graph_data.join(',')}&chdl=Points&chco=5131C9&chxt=x&chxl=0:|0|1|2|3|5|8|13|20|40|100|Break Pls&chxs=0,000000,14,-1&chf=bg,s,FFFFFF|c,s,FFFFFF&chbh=a&chtt=Points%20Histogram&chts=000000,12&chds=a&chm=N,000000,0,-1,11`
      ...
      return false; // do not bubble event
    } else if (trigger.callback_id.match(/^select_point_action_modfib$/)) {
      var message = {
        user: trigger.user,
        channel: trigger.channel,
        text: '<@' + bot.identity.id + '> ' + trigger.actions[0].value,
